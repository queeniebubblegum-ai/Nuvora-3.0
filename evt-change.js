    ...
    setup: () => {
        document.body.addEventListener('change', (e) => {
            ...
            const changeMap = {
                ...
                'setReportCashflowPeriod': () => App.setReportCashflowPeriod(target.value),
                ...
                'filterTransactionSubgroups': () => {
                    ...
                    if (!sub) return;
                    const options = db.categorias.filter(c => c.ativo !== false && !c.arquivada && (!c.tipo || c.tipo === type) && (c.grupo || c.nome) === group);
                ...
                // Kept defensive for older cached markup/backups. The current
                // category form uses a free-text name and an optional group.
                'filterCategoryGroups': () => App.prepareCategoryParents(),
                'prepareCategoryParents': () => App.prepareCategoryParents(),
                'toggleCategoryLevel': () => App.toggleCategoryLevel(target.value),
                ...
                'handleCardCategoryGroup': () => {
                    ...
                    const options = db.categorias.filter(c => c.ativo !== false && !c.arquivada && (!c.tipo || c.tipo === 'despesa') && (c.grupo || c.nome) === target.value);
                    sub.innerHTML = '<option value="">Subgrupo (opcional)</option>' + options.map(c => `<option value="${Utils.escapeHTML(c.nome)}">${Utils.escapeHTML(c.subgrupo || c.nome)}</option>`).join('');
                ...
                'invoiceClassificationGroup': () => {
                    ...
                    if (!subgroup) return;
                    const group = target.value;
                    const categories = (db.categorias || []).filter(c => c.ativo !== false && !c.arquivada && (c.grupo || c.nome) === group && (!c.tipo || c.tipo === 'despesa'));
