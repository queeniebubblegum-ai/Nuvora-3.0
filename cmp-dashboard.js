...
export const DashboardComponents = {
    ...
    dashboardAgenda: (agendamentos = [], receitas = []) => {
        ...
        let cells = ''; for (let i=0;i<primeiro;i++) cells += '<div></div>'; for (let dia=1;dia<=totalDias;dia++) { const lista = porDia[dia] || []; cells += `<button data-action="showAgendaDay" data-payload="${ano}-${String(mes+1).padStart(2,'0')}-${String(dia).padStart(2,'0')}" class="min-h-[54px] p-1.5 rounded-lg border ${lista.length ? 'border-brand-medium/40 bg-brand-medium/5' : 'border-border'} text-left hover:bg-bg transition-colors"><span class="text-xs font-bold text-text-primary">${dia}</span>${lista.length ? `<span class="block mt-1 text-[9px] font-bold ${lista.some(i=>i.tipo==='receita') ? 'text-success' : 'text-danger'}">${lista.length} item(ns)</span>` : ''}</button>`; }
        return `<div class="bg-surface p-5 rounded-[16px] border border-border shadow-soft"><div class="flex justify-between items-center mb-4"><div><h3 class="font-bold text-text-primary text-lg font-primary">Agenda financeira</h3><p class="text-xs text-text-secondary">${nomes[mes]} de ${ano} · clique em um dia</p></div><i class="fa-regular fa-calendar text-brand-medium text-xl"></i></div><div class="grid grid-cols-7 gap-1.5 mb-2 text-center text-[9px] font-bold text-text-secondary"><span>DOM</span><span>SEG</span><span>TER</span><span>QUA</span><span>QUI</span><span>SEX</span><span>SÁB</span></div><div class="grid grid-cols-7 gap-1.5">${cells}</div></div>`;
    },
